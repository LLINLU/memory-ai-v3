...
export const useConversationState = (steps: Step[]) => {
  ...
  const handleHelpMeClick = () => {
    ...
    // Find the first message with the question
    ...
    if (firstMessageIndex !== -1) {
      ...
      const currentContent = conversationHistory[firstMessageIndex].content;
      
      // Add the help content as a new message
      setConversationHistory(prev => {
        const updated = [...prev];
        // Keep the original message with question and icon, just remove the button
        if (React.isValidElement(updated[firstMessageIndex].content)) {
          const originalContent = updated[firstMessageIndex].content;
          // Create a shallow copy of the message without modifying the question and icon
          updated[firstMessageIndex] = {
            ...updated[firstMessageIndex],
            content: originalContent
          };
        }
        
        // Add the help content as a separate message
        return [
          ...updated,
          { 
            type: "system", 
